    ...
    void Update() { 
        ...
        if (Input.GetKey("right")) { 
            ...
            if (Input.GetKeyDown("up") && isGrounded) { 
                ...
                isGrounded = false; 
                // Evitar múltiples saltos en el aire 
                gameObject.GetComponent<Rigidbody2D>().AddForce(new Vector2(0, 600f)); 
            } 

            if (Input.GetKey("space")) { 
                gameObject.GetComponent<Animator>().SetBool("attack", true); 
                isAttacking = true; } if (!Input.GetKey("space") && isAttacking) { 
                    gameObject.GetComponent<Animator>().SetBool("attack", false); 
            } 
        ...
        private void OnCollisionEnter2D(Collision2D collision) { 
            if (collision.gameObject.CompareTag("Suelo")) { 
                ...
                // Desactivar animación de salto 
                gameObject.GetComponent<Animator>().SetBool("launching", false); 
                //Desactivar la animacion de deslizarse si no se presiona la tecla 
                } 
            } 
